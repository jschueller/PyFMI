...
cdef class FMUModelME3(FMUModelBase3):
    ...
    def _get_nominal_continuous_states(self):
        ...
        if self._initialized_fmu == 0:
            # not legal to call CAPI here, fetch from XML instead.
            self._preinit_nominal_continuous_states = np.array([self.get_variable_nominal(var_name) for var_name in self.get_states_list().keys()])
            return self._preinit_nominal_continuous_states
